...
async def soundcloud(email, client, out):
    ...
    getAuth = await client.get(
        "https://soundcloud.com/octobersveryown", headers=headers
    )
    ...
    API = await client.get(
        f"https://api-auth.soundcloud.com/web-auth/identifier?q={linkMail}&client_id={clientId}",
        headers=headers,
    )
