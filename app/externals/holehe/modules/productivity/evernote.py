...
async def evernote(email, client, out):
    ...
    data2 = {
        "username": email,
        "evaluateUsername": "",
        "hpts": data.text.split('document.getElementById("hpts").value = "')[1].split(
            '"'
        )[0],
        "hptsh": data.text.split('document.getElementById("hptsh").value = "')[1].split(
            '"'
        )[0],
        ...
        "_sourcePage": data.text.split(
            '<input type="hidden" name="_sourcePage" value="'
        )[1].split('"')[0],
        "__fp": data.text.split('<input type="hidden" name="__fp" value="')[1].split(
            '"'
        )[0],
    }
    response = await client.post(
        "https://www.evernote.com/Login.action", data=data2, headers=headers
    )
