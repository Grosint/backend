...
async def cracked_to(email, client, out):
    ...
    try:
        r = await client.get(
            "https://cracked.to/member.php", headers=headers, timeout=1
        )
        ...
        response = await client.post(
            "https://cracked.to/xmlhttp.php", headers=headers, params=params, data=data
        )
        if (
            "Your request was blocked" not in response.text
