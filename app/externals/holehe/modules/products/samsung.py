...
async def samsung(email, client, out):
    ...
    if response.status_code == 200:
        if (
            "rtnCd" in data.keys()
            and "INAPPROPRIATE_CHARACTERS" not in response.text
            and "accounts aren't supported." not in response.text
        ):
...
async def get_phone_number(email, client, cookies, headers):
    ...
    if data["rtnCd"] == "NEXT":
        req = await client.get(
            "https://account.samsung.com" + data["nextURL"],
            headers=headers,
            cookies=cookies,
        )
        ...
        if found:
            ...
            response = await client.post(
                "https://account.samsung.com/accounts/v1/DCGLIT/resetPasswordWithRecoveryProc",
                headers=headers,
                params=params,
                cookies=cookies,
            )
