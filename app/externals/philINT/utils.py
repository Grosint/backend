...
def extract_links_and_emails(text: str) -> tuple[list]:
    ...
    clean_links_images = list(
        set([(el[0] + "s:" + el[1]).lower() for el in links_images])
    )
...
def import_submodules(package="app.externals.philINT.modules"):
    """
...
def all_data_to_dict(all_data):
    ...
    return {
        type(el).__name__: {
            el._fields[i]: el[i] for i in range(len(el)) if el[i] is not None
        }
        for el in all_data
    }
...
def display_gpx_dataframe(gpx_dataframe: pandas.DataFrame, save: bool = False):
    ...
    coordinates = [
        tuple(x) for x in gpx_dataframe[["latitude", "longitude"]].to_numpy()
    ]
