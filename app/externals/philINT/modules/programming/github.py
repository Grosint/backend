...
import app.externals.philINT.utils
...
GitHubData = namedtuple(
    "GitHub",
    [
        "Username",
        "Github_profile",
        "Github_gists_profile",
        "Picture",
        "Pictures",
        "Links",
        "Emails",
...
async def from_email(email):
    ...
    if response["total_count"] == 1:
        ...
        print(
            "GitHub : Multiple users display the same email address, can't pivot accurately"
        )
        return None
...
async def username_to_readme(username):
    async with httpx.AsyncClient() as client:
        resp = await client.get(
            endpoint_config["GitHub"]["ENDPOINT_USERNAME"] + username + "/repos"
        )
    repos = resp.json()
    if repos != []:
        for el in repos:
...
async def from_username(username):
    async with httpx.AsyncClient() as client:
        resp = await client.get(
            endpoint_config["GitHub"]["ENDPOINT_USERNAME"] + username
        )
    user_data = resp.json()
    ...
    if readme is not None:
        readme_data = app.externals.philINT.utils.extract_links_and_emails(readme)
    else:
        readme_data = None
    return GitHubData(
        ...
        Github_gists_profile=(
            user_data["html_url"][0:8] + "gists." + user_data["html_url"][8:]
            if user_data["public_gists"] != 0
            else None
        ),
        Picture=user_data["avatar_url"],
        Pictures=(
            readme_data[1] if readme_data is not None and readme_data[1] != [] else None
        ),
        Links=(
            readme_data[0] + [user_data["blog"]]
            ...
            and user_data["blog"] != []
                ...
                if user_data["blog"] != ""
                    ...
                    readme_data[0]
                    if readme_data is not None and readme_data[0] != []
                    else None
                )
            )
        ),
        Emails=(
            readme_data[2] + [user_data["email"]]
            if readme_data is not None
            and readme_data[2] != []
            and user_data["email"] != []
            else (
                ...
                if user_data["email"] != ""
                    ...
                    readme_data[2]
                    if readme_data is not None and readme_data[2] != []
                    else None
                )
            )
        ),
        Gravatar_id=(
            user_data["gravatar_id"] if user_data["gravatar_id"] != "" else None
        ),
