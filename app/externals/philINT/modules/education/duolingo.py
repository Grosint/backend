...
Course = namedtuple(
    "Duolingo_Course", ["Language", "Learning_from", "Crowns", "Completion", "Exp"]
)
    ...
    results = [
        DuolingoData(
            ...
            Picture=(
                ...
                if "default" not in user["picture"]
                else None
            ),
            ...
            Country=(
                ...
                if "profileCountry" in user and user["profileCountry"] is not None
                else None
            ),
...
async def from_email(email):
    async with httpx.AsyncClient() as client:
        response = await client.get(
            endpoint_config["Duolingo"]["ENDPOINT_EMAIL"] + email,
...
async def from_username(username):
    async with httpx.AsyncClient() as client:
        response = await client.get(
            endpoint_config["Duolingo"]["ENDPOINT_USERNAME"] + username,
            headers={"User-Agent": endpoint_config["BASE"]["USER_AGENT"]},
        )
    return duolingo_response_parser(response.json())
