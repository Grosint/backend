...
LinkedAccount = namedtuple(
    "Gravatar_Linked_Account",
    ["Domain", "Display_name", "URL", "Username", "Website_name"],
)
...
async def from_email(email):
    ...
    async with httpx.AsyncClient() as client:
        resp = await client.get(
            endpoint_config["Gravatar"]["ENDPOINT_EMAIL"] + email_md5 + ".json",
    ...
    return [
        GravatarData(
            ...
            Currency=(
                [
                    Currency(Type=curr["type"], Wallet=curr["value"])
            ...
            URLs=(
                [url["value"] for url in el["urls"]]
                if "urls" in el and len(el["urls"]) != 0
                else None
            ),
        )
