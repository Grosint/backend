...
credentials_config.read(str(philint_root / "credentials.conf"))

ChessData = namedtuple(
    "Chess",
    [
        "Picture",
        "Profile",
...
async def from_email(email):
    ...
    return await from_username(
        user_data.get_attribute("innerHTML").split("username")[1].split('"')[1]
    )
...
async def from_username(username):
    async with httpx.AsyncClient() as client:
        resp = await client.get(
            endpoint_config["Chess"]["ENDPOINT_USERNAME"] + username.lower(),
    ...
    return ChessData(
        ...
        Account_creation_date=datetime.datetime.utcfromtimestamp(
            response["joined"]
        ).strftime("%Y-%m-%d %H:%M:%S"),
    )
