import app.externals.philINT.utils as utils
...
class Email:
    ...
    async def run_all_coro(self):
        ...
        Works with jupyter notebooks.
        """
        # Try EVA service, but don't fail if it errors - continue with other modules
        try:
            await self.get_additionnal_info_eva_coro()
        except Exception:
            # Don't raise - continue with other modules even if EVA fails
            pass
...
class Person:
    ...
    def fill_from_email(self, email: Email):
        ...
        """
        # Handle case where connections is None (e.g., after network errors)
        if email.connections is None:
            email.connections = []
        ...
        self.email_addresses = list(
            set(
                ...
                + [
                    ...
                    if "Mails" in connection._fields and connection.Mails is not None
                ]
            )
        )
        self.usernames = list(
            set(
                ...
                + [
                    connection.Username
                    for connection in email.connections
                    if "Username" in connection._fields
                    and connection.Username is not None
                ]
            )
        )
        self.names = list(
            set(
                ...
                + [
                    connection.Name
                    for connection in email.connections
                    if "Name" in connection._fields and connection.Name is not None
                ]
            )
        )
        self.phone_numbers = list(
            set(
                ...
                + [
                    ...
                    for connection in email.connections
                    if "Phone_number" in connection._fields
                    and connection.Phone_number is not None
                ]
            )
        )
        self.all_data = utils.remove_duplicate_unhashable(
            self.all_data + email.connections
        )
        self.accounts = list(
            set(
                self.accounts
                + [
                    "Chess.com" if type(el).__name__ == "Chess" else type(el).__name__
                    for el in email.connections
                ]
            )
        )
        pics = []
    ...
    def fill_from_username(self, username: Username):
        ...
        self.all_data = utils.remove_duplicate_unhashable(
            self.all_data + username.connections
        )
        self.accounts = list(
            set(
                self.accounts
                + [
                    "Chess.com" if type(el).__name__ == "Chess" else type(el).__name__
                    for el in username.connections
                ]
            )
        )
        pics = []
    ...
    def export_raw_data(self, file_name: str = "data_export"):
        ...
        """
        json = utils.all_data_export(self.all_data, file_name)
        return json
