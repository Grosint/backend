...
curl -f http://localhost:8000/api/health 2>/dev/null && echo "✅ Healthy" || echo "❌ Unhealthy"
...
docker-compose -f /opt/grosint-monitoring/docker-compose.logs.yml exec -T prometheus du -sh /prometheus 2>/dev/null || echo "Prometheus data not accessible"
docker-compose -f /opt/grosint-monitoring/docker-compose.logs.yml exec -T loki du -sh /loki 2>/dev/null || echo "Loki data not accessible"
echo -e "\nDisk Usage:"
